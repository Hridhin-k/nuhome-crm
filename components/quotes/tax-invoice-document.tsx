import { PrintQuoteButton } from "@/components/quotes/print-quote-button";
import { formatInrExact } from "@/lib/format/money";
import { lineTaxable } from "@/lib/gst";

type InvoiceItem = {
  description: string;
  quantity: number | string;
  unit_price: number | string;
  discount: number | string;
  tax: number | string;
  line_total: number | string;
  hsn_code: string | null;
  gst_rate: number | string;
};

export function TaxInvoiceDocument({
  invoiceNumber,
  issuedAt,
  company,
  customer,
  quoteNumber,
  orderNumber,
  version,
  items,
}: {
  invoiceNumber: string;
  issuedAt: string | null;
  company: {
    legal_name: string;
    gstin: string | null;
    address: string | null;
    phone: string | null;
    email: string | null;
  };
  customer: {
    name: string;
    phone: string | null;
    email?: string | null;
    gstin: string | null;
    billing_address: string | null;
    site_address: string | null;
    address: string | null;
  } | null;
  quoteNumber: string;
  orderNumber?: string;
  version: {
    version_number?: number;
    subtotal: number | string;
    discount: number | string;
    tax: number | string;
    total: number | string;
    notes?: string | null;
  } | null;
  items: InvoiceItem[];
}) {
  const issued = issuedAt
    ? new Date(issuedAt).toLocaleDateString("en-IN", {
        day: "numeric",
        month: "long",
        year: "numeric",
      })
    : new Date().toLocaleDateString("en-IN", {
        day: "numeric",
        month: "long",
        year: "numeric",
      });
  const billing =
    customer?.billing_address || customer?.address || null;
  const site = customer?.site_address || billing;
  const totals = version ?? {
    subtotal: 0,
    discount: 0,
    tax: 0,
    total: 0,
  };

  return (
    <article className="customer-quote mx-auto max-w-3xl overflow-hidden rounded-lg border border-outline-variant bg-white px-6 py-8 shadow-card print:border-0 print:shadow-none md:px-12 md:py-12">
      <header className="border-b border-outline-variant pb-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-headline-lg tracking-tight text-primary">
              {company.legal_name}
            </p>
            <p className="mt-1 text-headline-md text-secondary">Tax Invoice</p>
            {company.gstin ? (
              <p className="mt-2 text-body-sm text-on-surface-variant">
                GSTIN {company.gstin}
              </p>
            ) : null}
            {company.address ? (
              <p className="mt-1 max-w-xs text-body-sm text-on-surface-variant">
                {company.address}
              </p>
            ) : null}
            {company.phone || company.email ? (
              <p className="mt-1 text-body-sm text-on-surface-variant">
                {[company.phone, company.email].filter(Boolean).join(" · ")}
              </p>
            ) : null}
          </div>
          <div className="space-y-1 text-right text-data-tabular text-on-surface-variant">
            <p>
              <span className="text-secondary">Invoice: </span>
              <span className="font-semibold text-on-surface">
                {invoiceNumber}
              </span>
            </p>
            {orderNumber ? (
              <p>
                <span className="text-secondary">Order: </span>
                <span className="font-semibold text-on-surface">
                  {orderNumber}
                </span>
              </p>
            ) : null}
            <p>
              <span className="text-secondary">Quote: </span>
              <span className="font-semibold text-on-surface">{quoteNumber}</span>
            </p>
            <p>
              <span className="text-secondary">Date: </span>
              <span className="font-semibold text-on-surface">{issued}</span>
            </p>
          </div>
        </div>
      </header>

      <section className="mt-8 grid gap-4 sm:grid-cols-2">
        <div className="rounded border border-outline-variant bg-surface-bright p-3">
          <p className="text-label-caps text-secondary">Bill to</p>
          <p className="mt-2 text-subheading">{customer?.name ?? "Customer"}</p>
          {customer?.gstin ? (
            <p className="text-body-sm text-on-surface-variant">
              GSTIN {customer.gstin}
            </p>
          ) : null}
          {customer?.phone ? (
            <p className="text-body-sm text-on-surface-variant">
              {customer.phone}
            </p>
          ) : null}
          {billing ? (
            <p className="mt-1 whitespace-pre-wrap text-body-sm text-on-surface-variant">
              {billing}
            </p>
          ) : null}
        </div>
        <div className="rounded border border-outline-variant bg-surface-bright p-3">
          <p className="text-label-caps text-secondary">Ship to / site</p>
          <p className="mt-2 text-subheading">{customer?.name ?? "Customer"}</p>
          {site ? (
            <p className="mt-1 whitespace-pre-wrap text-body-sm text-on-surface-variant">
              {site}
            </p>
          ) : (
            <p className="mt-1 text-body-sm text-on-surface-variant">
              Same as billing
            </p>
          )}
        </div>
      </section>

      <section className="mt-10 overflow-x-auto">
        <table className="w-full min-w-[640px] text-sm">
          <thead>
            <tr className="border-b border-outline-variant text-left text-on-surface-variant">
              <th className="pb-2 font-medium">Item</th>
              <th className="pb-2 font-medium">HSN</th>
              <th className="pb-2 text-right font-medium">Qty</th>
              <th className="pb-2 text-right font-medium">Rate</th>
              <th className="pb-2 text-right font-medium">Taxable</th>
              <th className="pb-2 text-right font-medium">GST</th>
              <th className="pb-2 text-right font-medium">Amount</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-outline-variant">
            {items.map((item, index) => {
              const taxable = lineTaxable(
                Number(item.quantity),
                Number(item.unit_price),
                Number(item.discount),
              );
              const rate = Number(item.gst_rate) || 0;
              return (
                <tr key={`${item.description}-${index}`}>
                  <td className="py-3 pr-3">{item.description}</td>
                  <td className="py-3 pr-3 text-on-surface-variant">
                    {item.hsn_code ?? "—"}
                  </td>
                  <td className="py-3 text-right tabular-nums">{item.quantity}</td>
                  <td className="py-3 text-right tabular-nums">
                    {formatInrExact(Number(item.unit_price))}
                  </td>
                  <td className="py-3 text-right tabular-nums">
                    {formatInrExact(taxable)}
                  </td>
                  <td className="py-3 text-right tabular-nums">
                    {rate > 0
                      ? `${rate}% · ${formatInrExact(Number(item.tax))}`
                      : formatInrExact(Number(item.tax))}
                  </td>
                  <td className="py-3 text-right tabular-nums">
                    {formatInrExact(Number(item.line_total))}
                  </td>
                </tr>
              );
            })}
          </tbody>
        </table>
      </section>

      <section className="mt-8 border-t border-outline-variant pt-5">
        <dl className="ml-auto max-w-xs space-y-2 text-sm">
          <div className="flex justify-between">
            <dt className="text-on-surface-variant">Taxable value</dt>
            <dd className="tabular-nums">
              {formatInrExact(Number(totals.subtotal) - Number(totals.discount))}
            </dd>
          </div>
          {Number(totals.discount) > 0 ? (
            <div className="flex justify-between">
              <dt className="text-on-surface-variant">Discount</dt>
              <dd className="tabular-nums">
                {formatInrExact(Number(totals.discount))}
              </dd>
            </div>
          ) : null}
          <div className="flex justify-between">
            <dt className="text-on-surface-variant">GST</dt>
            <dd className="tabular-nums">
              {formatInrExact(Number(totals.tax))}
            </dd>
          </div>
          <div className="flex justify-between border-t border-outline-variant pt-2 text-base font-semibold">
            <dt>Total</dt>
            <dd className="tabular-nums">
              {formatInrExact(Number(totals.total))}
            </dd>
          </div>
        </dl>
      </section>

      <footer className="mt-10 border-t border-outline-variant pt-5 text-sm text-on-surface-variant">
        <p>
          This is a GST tax invoice generated from order{" "}
          {orderNumber ?? quoteNumber}. Subject
          to the showroom&apos;s terms.
        </p>
      </footer>

      <PrintQuoteButton />
    </article>
  );
}
