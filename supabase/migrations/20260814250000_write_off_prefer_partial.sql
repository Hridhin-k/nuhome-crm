-- Apply shortage to the vendor batch that already has a partial receipt
-- before touching other open POs (same-millisecond created_at is common).
...
begin
  ...
  loop
    ...
    for open_line in
      ...
      order by
        case when voi.quantity_received > 0 then 0 else 1 end,
        case vo.status when 'dispatched' then 0 when 'sent' then 1 else 2 end,
        vo.created_at,
        voi.id
    loop
